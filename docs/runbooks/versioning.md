...
- **`uv run --no-sync ruff` skips all seven guards** that `make lint` bundles (`check_file_sizes.py`,
  `check_layer_imports.py`, `check_doc_links.py`, `check_control_liveness.py`,
  `check_doc_claims.py`, `check_state_keys.py`, `check_migration_chain.py`). If you dodge the
  dev-server exe lock that way, run the seven explicitly — they are the half of `make lint` that
  `ruff` alone never runs.
...
backfilled after the fact — releases before 2026-08 were recorded in the CHANGELOG only. `v0.6.3`
was tagged 2026-09-02 (on the release commit `253fa343`, two days after the fact — 0.6.1 and 0.6.2
were never tagged at all) and is the first release published to the public mirror as a tag, so it is
the first version the one-liner installs pinned. **The lesson this history teaches: §5 step 6 is the
step that gets skipped. Tag in the same sitting as the merge.**
