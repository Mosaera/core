...
- **[debt] Operator friction — the control exists, but not when you need it (2026-08-24)** — five
  ...
  - ~~**STILL OPEN, both from #116:** the write-mode toggle silently does nothing while a run is
    paused, and the run page's live stream dies after every approval~~ — **DONE 2026-09-02**
    (alpha wave 1). The stream resumes via per-frame ids + `Last-Event-ID` instead of replaying,
    an idle park heartbeats so proxies stop killing it, and the mode endpoint + workbench say
    explicitly whether a switch reached the open gate or only the next one (the parked gate is
    deliberately NOT auto-resolved — approval semantics untouched). Live network-drop
    verification on a deployed instance still owed.

- **[arc] Alpha productization (2026-09-02)** — executing the alpha-readiness review's phases.
  Phase 0 DONE: `v0.6.3` tagged on the release commit, published to the public mirror (the first
  release the one-liner installs pinned), CHANGELOG `[Unreleased]` restored, five ADR statuses
  corrected. Wave 1 DONE (four parallel worktrees, merged sequentially, gates green at each step):
  installer/wizard exit truth (G3/G4/1C/D1), Ollama-first-class models + settings truth (M1, O1–O5,
  S1/S2/S4), guided-run realtime (#116 residuals), git-workflow coherence (S3/S4, F1–F3, F8–F12,
  F15, 4E). Owed from wave 1: the stranger test (owner), live-forge validations (GitHub status
  probe/disconnects/installation repos/clone-reset/compose-patch on a real PR), live network-drop
  SSE verification, and a follow-up: a **manual/guided** launch that hits base drift surfaces a
  bare 400 with no recovery UI (only the autonomous sweep writes `project.error` —
  `app_context/_launch.py:339` vs `:405`); DriftRecovery is unreachable on the guided path.
  Waves 2–3 DONE same day: first-contact UX (one front door, consolidated needs-you band, shared
  error/retry + 404, plain-language outcome tokens), tab consolidation 9 → 6 (redirects kept),
  `pm_chat_tools` on by default + honest chips (ADR-0111), the Playwright smoke path
  (`npm --prefix apps/web run e2e` — real Chromium/API/throwaway Postgres; CI wiring owed, the
  job spec is in the wave-2 agent summary), and the two user guides
  (`docs/user-guide/{core-workflow,troubleshooting-runs}.md`). Still owed beyond wave 1's list:
  ~~deploy staging + live-forge validations~~ — **DONE 2026-09-03**
  ([record](engineering-history/live-forge-validation-2026-09-03.md)): a real GitHub PR by
  mosaera[bot] with faithful compose (ADR-0114 criterion 4), the ADR-0125 local-first publish +
  credentialed finish, liveness probes live; two live-found defects fixed same day (GitLab-blind
  readiness `ab78855f`; the OAuth-app instructions registered one of two callbacks `90d714fb`).
  Still owed: the stranger test; a CODEOWNERS review of the Phase-0 ADR-status edits; GitLab's
  first-time consent click; disconnect presses; live drift-reset against a real moved remote.
...
- **[debt] Fails closed when exposed — the defaults are dev-shaped (2026-08-28)** — four findings
  ...
  - **#123 — SHIPPED 2026-08-31**
    ([ADR-0126](adr/ADR-0126-the-wizard-mints-the-key-that-encrypts-at-rest.md)) an exposed bind
    requires `MOSAERA_SECRET_KEY`. GitLab and BYOM credentials are plaintext without it (`0600` is a
  ...
  - **#124 — SHIPPED 2026-08-31**
    ([ADR-0126](adr/ADR-0126-the-wizard-mints-the-key-that-encrypts-at-rest.md)) an exposed bind
    must declare its TLS posture. **Not** "force `Secure` on a non-loopback bind" — that silently breaks every
  ...
  - **Status 2026-08-31:** #123 + #124 shipped with owner approval, together with a third change
    that was not on this list and is the one that closes the gap on the path that has users: the
    wizard now **mints** `MOSAERA_SECRET_KEY` for every install (ADR-0126, amending ADR-0039).
    Before it, a default install stored the GitLab PAT, OAuth client secret, GitHub App private key
    and every BYOM key in plaintext under `0600`, and nothing in setup mentioned it.
    **`red-team: done`** — 3 rounds scoped to the change, 4 FIX-NOW (all fixed and pinned) + 2 false-positive; the STOP rule fired on *presence is not usability*, logged in ADR-0126. #122 and #125 remain open.
...
- **[arc] Efficiency + over-park — the dominant cause is WRONG evidence, not missing evidence
  ...
  - **Slice 5 — the bar is visible BEFORE it lands, and nothing looks at it
    (#130, opened 2026-09-02)** — First live run on `0.6.3` (LedgerCLI, `20260902-125337-1443d3`,
    cancelled at the first write gate) proposed **55 assertions across 13 test functions for one
    boolean flag**, pinning `last_line.startswith("count: ")` when the brief said only "report how
    many entries it printed". `count: ` is nowhere in the task; `3 entries` would have been correct
    and refused. **Cost to observe: 19 calls, 143k tokens, no code written** — against a delivery
    gate, where the same defect costs a whole run. TWO findings, deliberately separated:
    **(a) timing** — the payload is human-readable at the write gate, two stages before anything
    acts on it, and a guided operator is shown 10kB of assertions with nothing about whether they
    are meetable; **(b) detection** — running the SHIPPED detectors over that exact payload returns
    **0 findings**, so "surface what already exists" would surface nothing. The missing check is
    "a string literal absent from the trusted task, required as a substring/prefix of program
    OUTPUT" (MCB-01's shape too) — a general faithfulness rule, ADR-0085-permitted, but one that
    over-fires naively ("sum the amounts" → a test asserting `6`, also not in the spec), so it must
    be measured on the labelled corpus before shipping. Speak-never-act, per ADR-0062.
    Record: [`the-bar-is-visible-before-it-lands-2026-09-02.md`](engineering-history/the-bar-is-visible-before-it-lands-2026-09-02.md).
...
- **[arc] The PM can read the project's own record — [ADR-0111](adr/ADR-0111-pm-chat-may-read-its-own-ledgers.md)
  ...
  `MOSAERA_PM_CHAT_TOOLS` (**on by default since the 2026-09-02 Alpha flip**; off remains byte-identical to the single call it
...
- **[arc] Alpha-outsider stoppers — a stranger takes their own project to a merge (issues filed
  ...
  **Slice 9 — symmetric capability ([ADR-0125](adr/ADR-0125-gitlab-creates-repositories-too.md)), BUILT 2026-08-31.**
  GitLab creates repositories too, on the same flow: bound single-use state spent before any
  exchange, session re-check, server-derived name, push-before-repoint fail-closed. Two differences,
  both from the providers rather than the flows — **no second application** (GitLab's `api` scope,
  already required by ADR-0104, can create a project; GitHub needed an OAuth App because its App
  tokens are refused) and created **private** (`clone.py` authenticates a private clone on the
  configured GitLab host and cannot for GitHub, so each provider gets the most private option it
  can actually deliver rather than the worse of the two for symmetry).
  It also **finishes credentialed**: the same grant mints the project access token, so no connect
  step remains — the closest either provider gets to one-click publish.
  `_shared.push_repository_to` is now provider-neutral and both forges call it; a second copy of
  that hygiene is how one quietly loses a protection the other has. The project card offers
  whichever forge the instance can actually perform, not the ones it knows about.
  **OWED:** the GitLab authorize round-trip. The GitHub equivalent is live-validated; this inherits
  its shape but not its evidence.
...
**[prereq] Settings v2 — intent profiles over the knob surface — *untracked, issue proposal*** ([ADR-0122](adr/ADR-0122-core-settings-are-intent-profiles.md)) — `GENERAL_KNOBS` reached 80 entries, ~62 of them reachable in the Settings page. Three intent profiles (`effort_profile`, `quality_profile`, `verification_profile`) derive the mechanics; precedence gains a layer, `env > stored > profile > default`, with the profile BELOW stored so it can only fill what the operator never set.
...
- **Collapsed 4 profiles → 3, and closed a control that provably did nothing (2026-09-02).** `autonomy`+`recovery` both answered "how hard does it try?" and were indistinguishable in use — the source proposal contradicted itself about which owned the recovery knobs, which was the evidence the split was not real. They are one `effort_profile`, whose vocabulary now MATCHES the `reliability_sensitivity` dial (#51, ADR-0056) it drives. **The defect that forced this:** `apply_reliability_sensitivity` runs in `build_graph`, *after* the profile layer, so `recovery_profile=persistent` + `reliability_sensitivity=cautious` measured `max_escalations=0` — the opposite of the selection, silently. Two dials on one axis sharing the words *balanced* and *persistent*. `SENSITIVITY_OWNED` now names the six knobs that function overwrites, no profile may derive one, and two regression tests pin it including an end-to-end assertion through `build_graph`.
- **The profiles are described, not promised.** Each derived knob carries an `EFFECTS` sentence and the settings page shows every option side by side, listing only what DIFFERS — a row reading `max_reason_attempts: 3` predicts nothing, which is what made the feature read as theatre. The page also names what NO profile changes (`NEVER_DERIVED`) and flags settings the operator has overridden. **No outcome claim is made anywhere**, deliberately: on the ADR-0081 ladder these sit at C0/C1, and grounding a delivery-rate claim needs ~390 runs/arm at 10pp against a 26-case corpus with undecided case-vs-run independence. Cost and wall-clock are the cheap axes and the bench already records them — that measurement is OWED before any label implies an outcome.
- **A finished run STAMPS the profiles it started with.** `runner/_provenance.py` captures them once at run start and never re-reads (the same rule the control roster already followed), and the record footer shows them beside the run's own cost, revisions and duration. This is the cheapest honest answer to "what should I expect": a profile stops being a general promise and becomes an observation about THIS run. It needed no migration and no A/B — the data was already recorded.
