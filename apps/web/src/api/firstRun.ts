...
/** What a preset would resolve to for one role, and WHY. Empty `model` = unresolved, which is a
 *  first-class outcome the wizard shows rather than hides. */
...
export interface PresetPreview {
  id: string;
  locality: "on_box" | "any";
  prefer: "cheapest" | "operator";
  summary: string;
  resolution: PresetRow[];
}

export interface SetupPresets {
  inventory: { provider: string; model: string; on_box: boolean; price: number | null }[];
...
export const firstRunApi = {
  ...
  setupPresets: () => apiFetch("/api/setup/presets").then(json<SetupPresets>)
};
