...
export interface DeliveryCapability {
  ...
  note?: string;
  /** task 4F (F8/F9/F10): the ONE next step that resolves `detail`, when there is one — today
   *  only the "not on any forge" refusal has one (publish it). `pane` names the project
   *  settings pane to link to. Absent on a server predating this, or when no single action
   *  fixes the refusal (a credential problem already has its own remedy text). */
  action?: { label: string; pane: string } | null;
...
export const deliveryApi = {
   ...
   *
   *  `verified`/`verify_error` (task 4B-i, F1/F2): `configured` is presence only (id + key
   *  set); `verified` is null until credentials are present, then true/false from a real
   *  authenticated call to GitHub, cached server-side ~60s. */
  githubStatus: () =>
  ...
  /** Clear the instance's stored GitHub App credentials (task 4B-ii). A no-op for an
   *  env-pinned App — env still wins (ADR-0005). */
  disconnectGithubApp: () =>
    adminFetch(`/api/github/app/disconnect`, { method: "POST" }).then(json<{ ok: boolean }>),

  /** Forget one project's resolved GitHub installation id (task 4B-ii). Cache-only: delivery
   *  re-resolves from GitHub on every call regardless (ADR-0114). */
  disconnectProjectGithub: (id: string) =>
    adminFetch(`/api/projects/${id}/github/disconnect`, { method: "POST" }).then(
      json<{ disconnected: boolean }>,
    ),

  /** Repositories one App installation can reach — read-only (task 4C). New Project accepts
   *  any of these URLs; this panel does not itself start a project. */
  githubInstallationRepositories: (installationId: number) =>
    adminFetch(`/api/github/installations/${installationId}/repositories`).then(
      json<{
        configured: boolean;
        repositories: { full_name: string; html_url: string; private: boolean }[];
        error: string | null;
      }>,
    ),

  /** A per-project GitLab liveness probe (task 4B-i) — spends the project's OWN token on one
   *  cheap authenticated read, rather than trusting the stored presence bit. */
  gitlabRecheck: (id: string) =>
    adminFetch(`/api/projects/${id}/gitlab/recheck`, { method: "POST" }).then(
      json<{ ok: boolean; verified: boolean; error: string | null }>,
    ),

  /** Force the project's persistent clone back onto its remote base, discarding any
   *  local-only commits (task 4D) — the guarded recovery for the "base drift: …" pause note.
   *  409s while a run is active (the same clone-mutex pattern as every other mutating
   *  delivery route). */
  resetProjectClone: (id: string) =>
    adminFetch(`/api/projects/${id}/clone/reset`, { method: "POST" }).then(
      json<{ reset: boolean; detail: string }>,
    ),

  /** Clear a project's GitLab token pair (task 4B-ii). */
  disconnectProjectGitlab: (id: string) =>
    adminFetch(`/api/projects/${id}/gitlab/disconnect`, { method: "POST" }).then(
      json<{ disconnected: boolean }>,
    ),
  ...
  /** Whether GitLab can create a repository for a project (ADR-0125). Unlike GitHub this needs
   *  no second application — the `api` scope ADR-0104 already asks for can create a project. */
  gitlabRepoStatus: () =>
    apiFetch(`/api/gitlab/repo/status`).then(
      json<{ configured: boolean; is_admin: boolean; host: string }>,
    ),
