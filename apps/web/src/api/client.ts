...
export const api = {
  ...
  getGeneralSettings: () =>
    apiFetch("/api/settings/general").then(json<{ knobs: Record<string, KnobView> }>),
  saveGeneralSettings: (values: Record<string, KnobValue>) =>
    adminFetch("/api/settings/general", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ values }),
    }).then(json<{ knobs: Record<string, KnobView> }>),
  ...
  setRunMode: (id: string, mode: string) =>
    ...
    }).then(json<{ run_id: string; mode: string; previous: string }>),
...
/** Bindable models grouped by the provider they come from (Ollama live via
 *  /api/tags; hosted BYOM providers contribute curated suggestions). Powers the
 *  pricing and role-model pickers. */
