...
describe("the project Integration pane", () => {
  ...
  it("offers create-and-push for a project whose code is on disk", async () => {
    ...
    const cta = await screen.findByRole("button", { name: "Create and push" });
    // A full-page navigation, not a fetch — the session cookie must ride the redirect.
    expect(cta).toHaveAttribute("href", "/api/oauth/github/start?project_id=p1");
    expect(screen.getByText(/pushes this project's history/)).toBeInTheDocument();
    expect(screen.getByText(/Public repositories only/)).toBeInTheDocument();
  });

  it("does not offer it once the project already has a repository", async () => {
    mocks.githubRepoStatus.mockResolvedValue({
      configured: true,
      is_admin: true,
      host: "github.com",
  ...
  it("does not dead-end a project that is not on a forge", async () => {
    mocks.githubRepoStatus.mockResolvedValue({
      configured: false,
      is_admin: true,
      host: "github.com",
    ...
    expect(await screen.findByRole("heading", { name: "GitHub" })).toBeInTheDocument();
    expect(screen.getByText(/not on a forge yet/)).toBeInTheDocument();
    // With repo creation unconfigured the offer is withheld rather than dangled.
    expect(screen.queryByRole("button", { name: "Create and push" })).not.toBeInTheDocument();
  });
});
