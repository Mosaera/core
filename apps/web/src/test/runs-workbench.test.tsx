...
const setRunMode = vi.fn((_id: string, _mode: string) =>
  Promise.resolve({
    run_id: "r1",
    mode: "auto",
    previous: "ask",
    currently_parked: false,
    effective: "immediate" as "immediate" | "next_gate",
  }),
);
vi.mock("../api/client", async (importOriginal) => {
  ...
  return {
    ...
    api: {
      ...
      setRunMode: (id: string, mode: string) => setRunMode(id, mode),
    },
  };
});
...
describe("mode switch in front of an open gate (D3, #116)", () => {
  // writes_auto() is only consulted BEFORE a gate parks (runner/_loop.py), so a switch made
  // while a gate is already open cannot reach back and resolve it — it takes hold at the run's
  // NEXT gate. The endpoint says so (`currently_parked`/`effective`); the workbench must say so
  // too, instead of letting the moved toggle imply the open gate was just resolved.
  const parkedRun = () =>
    snap({
      status: "awaiting_approval",
      pending_interrupt: { id: "g1", value: { action: "deliver", summary: "approve delivery?" } },
    });

  it("names the open gate as unaffected when the switch only reaches the next one", async () => {
    getRun.mockResolvedValue(parkedRun());
    runDetail.mockResolvedValue(detail({ status: "AWAITING_APPROVAL" }));
    setRunMode.mockResolvedValue({
      run_id: "r1",
      mode: "auto",
      previous: "ask",
    ...
    renderWb();
    // Wait for the gate itself to be on screen (the mode row renders before the poll resolves;
    // clicking that early wouldn't know WHICH gate is open, which isn't this scenario — an
    // operator only reaches for the mode switch once they can see the gate in front of them).
    await screen.findByRole("button", { name: "Review" });
    fireEvent.click(screen.getByRole("button", { name: "auto" }));
    await waitFor(() => expect(setRunMode).toHaveBeenCalledWith("r1", "auto"));
    expect(await screen.findByText(/applies from the next gate/i)).toBeTruthy();
  });

  it("stays silent when the switch reached immediately (no gate was open)", async () => {
    getRun.mockResolvedValue(snap({ status: "running" }));
    runDetail.mockResolvedValue(detail({ status: "RUNNING" }));
    setRunMode.mockResolvedValue({
      run_id: "r1",
      mode: "auto",
      previous: "ask",
    ...
    await waitFor(() => expect(setRunMode).toHaveBeenCalledWith("r1", "auto"));
    expect(screen.queryByText(/applies from the next gate/i)).toBeNull();
  });
});
