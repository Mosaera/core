...
describe("delivery capability", () => {
  ...
  it("says so honestly when the host is not a recognized forge at all, and links to publish it", async () => {
    // task 4F (F8/F9/F10): the refusal used to be a dead end — no link to the one thing that
    // fixes it (publishing this project to a remote from the Integration pane).
    mocks.projectDeliveryCapability.mockResolvedValue({
      provider: "unknown",
      can_finish: false,
      reason: "not_gitlab",
      detail: "Delivery has nowhere to open a request.",
      action: { label: "Publish this project to a remote", pane: "integration" },
      has_gitlab_token: false,
      has_gitlab_api_token: false,
      merge_state_readable: false,
    });
    ...
    const cta = await screen.findByRole("button", { name: "Publish this project to a remote" });
