...
describe("ProjectOverview", () => {
  ...
  it("opens the full brief in a drawer from the charter band", () => {
    ...
    expect(within(drawer).getByRole("button", { name: "Open in Artifacts" })).toHaveAttribute(
      "href",
      "/projects/p1/delivery?view=artifacts",
    );
  });

  it("shows clean empty states for a fresh active project", async () => {
    ...
    // The consolidated "needs you" band (5C) says so once, quietly — but only once the
    // server-derived decisions have actually resolved with nothing, never before (a synchronous
    // "nothing needs you" ahead of that answer would be exactly the kind of premature all-clear
