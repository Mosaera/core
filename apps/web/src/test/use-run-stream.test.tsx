...
describe("useRunStream", () => {
  ...
  it("D4: a browser-driven reconnect flips connected back true (an 'open' fires on it)", async () => {
    getRun.mockResolvedValue(snap({ status: "running" }));
    const { result } = renderHook(() => useRunStream("r1"), { wrapper });
    await waitFor(() => expect(result.current.status).toBe("running"));
    const es = FakeEventSource.instances[0];
    act(() => es.listeners["error"]?.({ data: undefined }));
    await waitFor(() => expect(result.current.connected).toBe(false));
    // The browser's own retry re-established the connection — EventSource fires "open" again.
    // Before this fix nothing ever set connected back to true, so a fully-recovered stream kept
    // reporting itself lost forever.
    act(() => es.listeners["open"]?.({}));
