...
describe("deriveReadiness", () => {
  ...
  it("a GitHub App connection credentials readiness — no-token was GitLab-blind", () => {
    // Live finding 2026-09-03: a connected GitHub project (has_gitlab_token always false)
    // fell through to "no-token", so the open-PR action never rendered and ADR-0114's
    // criterion 4 was unreachable from the UI. capability.has_github_connection rides in
    // as the fourth argument.
    const p = project({ has_gitlab_token: false, runs: [run()] });
    expect(deriveReadiness(p, changes, null, true).state).toBe("ready");
    expect(deriveReadiness(p, { has_changes: true, remote_synced: false }, null, true).state).toBe(
      "delivered-unpushed",
    );
    expect(deriveReadiness(p, changes, null, false).state).toBe("no-token");
  });
