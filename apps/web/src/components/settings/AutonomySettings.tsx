...
const GROUPS: KnobGroup[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      // NOTE what is deliberately absent: `model_escalation_enabled` + `max_model_escalations`
      // are exposed AND set by the effort profile, but the escalation ladder they drive
      // (`role_escalation`) has no route/UI to populate — the toggle cannot fire (S2, readiness
      // review). The server marked both `internal` (config/_visibility.py); removed here rather
      // than left to silently vanish through KnobForm's visibility filter, which is the exact
      // "orphaned control" shape S1 flagged elsewhere in this file. Restore once the ladder is
      // configurable.
    ],
  },
  // NOTE what is deliberately absent: a "Tester & oracle" group used to live here
  // (tester_enabled, tester_step_limit, oracle_coverage, oracle_mutation_check). The whole
  // oracle stack is FORCED on for every autonomous run by `apply_oracle_posture` and is
  // `internal` server-side for exactly that reason (config/_visibility.py: "removing the
  // control is better than explaining why it is inert") — every field in that group silently
  // vanished through KnobForm's visibility filter (S1, readiness review). Deleted rather than
  // left as dead code presenting a heading with nothing under it.
];
