...
import { InstallationRepositories } from "../github/InstallationRepositories";
...
export function GitHubPanel() {
  ...
  // task 4B-i (F1/F2): `configured` is presence only; `verified` is a real authenticated
  // GET /app, so the three states an operator actually cares about are distinguishable.
  const verifiedLine = !appReady
    ? null
    : status?.verified === true
      ? "Credentials verified just now."
      : status?.verified === false
        ? `Credential rejected: ${status.verify_error || "unknown reason"}.`
        : "Not verified yet.";
  ...
  return (
    ...
    >
            ...
            {appReady ? (
              <>
                <p className="text-[12.5px] leading-relaxed text-muted-foreground">
                  Registered on this instance. Delivery signs as this app and mints a token scoped
                  to a single repository for each pull request.
                </p>
                {verifiedLine && (
                  <p
                    className={cn(
                ...
                <ul className="flex flex-col items-stretch overflow-hidden rounded-lg ring-1 ring-white/12">
                  {installations.map((inst) => (
                    <li
                      ...
                      className="flex flex-col gap-2 border-b border-border/40 px-3 py-2.5 last:border-0"
                    >
                      <button
                        type="button"
                        className="flex w-full items-center gap-3 text-left"
                        onClick={() =>
                          setExpanded((e) => (e === inst.id ? null : (inst.id ?? null)))
                        }
                      >
                      ...
                      {expanded === inst.id && inst.id != null && (
                        <InstallationRepositories installationId={inst.id} />
                      )}
                    </li>
                  ))}
                </ul>
      ...
      >
        <p>
          Clears this instance&rsquo;s stored App credentials. No project delivering to GitHub can
          push or open a pull request until an App is registered again. This does not uninstall
          the App on GitHub itself — do that from GitHub if you also want to revoke its access.
        </p>
      </ConfirmDialog>
    </ConnectionShell>
  );
