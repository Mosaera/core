 ...
 *
 *  While the capability query is in flight, or against an older server without the endpoint, it
 *  falls back to GitLab — the behaviour every project had before ADR-0112, so nothing regresses
 *  for the forge that was always assumed. */
function IntegrationPane({ project }: { project: Project }) {
  ...
  // An "unknown" provider means the source is not on a forge — usually a local path. That is
  // not a dead end: it is precisely the project that can have a repository created and its
  // history pushed into it (ADR-0120). `GitHubConnection` owns that offer and withholds it when
  // the instance cannot do it, so the honest card is the GitHub one, not a refusal.
  return <GitHubConnection project={project} capability={capability} />;
}
