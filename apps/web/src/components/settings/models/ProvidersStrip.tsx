      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {ordered.map((p) => (
          <ProviderCard
            key={p.id}
            provider={p}
            // Models a role currently binds to THIS provider — lets the card tell "reachable,
            // and everything it needs to serve is there" from "reachable, but a bound model
            // isn't" (O5), instead of a probe result that only ever says "connected".
            boundModels={Object.values(providers.roles)
