...
import { isNonChatModel, providerLabel } from "../../../lib/models";
              ...
              <Select value={model || null} onValueChange={(v) => pick(m.role, v ?? "")}>
                ...
                <SelectContent>
                  ...
                  {providers.sources.map((src) => {
                    // Every role here needs a chat/tool-calling model — an embedding model
                    // bound to one silently produces nonsense (O1-O3).
                    const chatModels = src.models.filter((mm) => !isNonChatModel(mm));
