...
function SetupStep({
  ...
  // Registered alongside it (one per line): the repo-create flow's own callback — see
  // GitLabOAuthApp.tsx for the live finding this closes.
  const createRedirectUri = `${(baseUrl || window.location.origin).replace(/\/$/, "")}/oauth/gitlab/create/callback`;
  const isSet =
    ...
    <>
      <div className="flex flex-col gap-3 px-4">
        ...
        <ol className="list-decimal space-y-1 pl-5 text-[12.5px] leading-relaxed text-muted-foreground marker:text-muted-foreground/70">
          ...
          <li>Paste BOTH redirect URIs below (one per line) and tick the {""}
        ...
        <div className="flex flex-col gap-1 rounded-md bg-muted/30 p-2.5 font-mono text-[11px] leading-relaxed text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <span className="shrink-0">create URI:</span>
            <span className="min-w-0 truncate text-foreground/90">{createRedirectUri}</span>
            <CopyButton
              text={createRedirectUri}
              label="Copy create redirect URI"
              className="ml-auto"
            />
          </div>
          <div className="flex items-center gap-1">
