...
export function MrComposeSheet({
  ...
  /** ADR-0112's delivery capability, when known. Drives which controls this sheet offers:
   *  GitHub has no squash/remove-source/labels at PR-create time and never merges from here
   *  (task 4A-ii) — those are GitLab-only concepts this sheet used to show unconditionally. */
...
function ComposeBody({
  ...
  return (
    <>
      <SheetHeader>
        <SheetTitle>Compose {nouns.request}</SheetTitle>
        <SheetDescription>
          {label} — review and edit before it's sent.
          {!isGithub && ` You still merge it in ${nouns.hostName}.`}
        </SheetDescription>
      </SheetHeader>

      <div className="flex flex-col gap-4 px-4 py-3">
        {!isGithub && !apiTokenPresent && (
          <p className="rounded-md bg-amber-500/10 px-3 py-2 text-[12px] leading-relaxed text-amber-600 dark:text-amber-400">
            ...
            the push-options transport, so title/body/labels are ignored and the {nouns.request}{" "}
            opens with the defaults. Add an api token in project settings to edit it faithfully.
          </p>
        )}
        ...
        {!isGithub && (
          <>
            ...
            <Toggle
              ...
              note={`${nouns.hostName} removes the branch once this merges. The commits live on in the target.`}
            />
          </>
        )}
      </div>
        ...
        <Button
          ...
          onClick={() =>
            onSubmit(draft.kind, {
              ...
              ...(apiTokenPresent && !isGithub && labelsTouched
                ? {
                    labels: labels
                      .split(",")
                      .map((l) => l.trim())
              ...
              ...(isGithub ? {} : { squash, remove_source_branch: removeSource }),
              ...(picked.size > 0 ? { commit_shas: [...picked] } : {}),
            })
          }
        >
          {busy ? "Opening…" : `Open ${nouns.request}`}
        </Button>
      </SheetFooter>
    </>
  );
