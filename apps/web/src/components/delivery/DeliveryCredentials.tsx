  ...
  if (provider !== "gitlab") {
    return (
      ...
      >
        ...
        {/* task 4F (F8/F9/F10): before this the refusal above named the problem and stopped —
            no link, so the fix (publish this project to a remote) was reachable only by
            already knowing the Integration pane exists. */}
        {capability?.action && (
          <Button
            size="sm"
            variant="outline"
            ...
            nativeButton={false}
            render={
              <Link to={`/projects/${project.id}/settings?pane=${capability.action.pane}`} />
            }
          >
