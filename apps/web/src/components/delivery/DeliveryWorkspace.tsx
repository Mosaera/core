...
export function DeliveryWorkspace({ project }: { project: Project }) {
  ...
  const readiness = deriveReadiness(project, diff, mr?.state ?? null);
  ...
  const [confirm, setConfirm] = useState<
    { kind: "prune" } | { kind: "branch"; name: string } | { kind: "retarget"; id: number } | null
  >(null);
    ...
    <div className="flex min-h-0 flex-col gap-4 lg:-mb-16 lg:h-[calc(100dvh-88px)] lg:min-h-[460px]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 items-start gap-3 lg:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] lg:items-stretch">
        ...
        <div className="flex min-h-0 flex-col gap-3 overflow-y-auto [scrollbar-color:var(--border)_transparent] [scrollbar-width:thin]">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              ...
              {(readiness.state === "mr-open" || readiness.state === "merged") &&
                project.mr_url && (
                  ...
                  >
                    {readiness.state === "merged" ? "View merged MR" : "View project MR"}
                  </Button>
                )}
            </div>
          </section>
          ...
          {(mergedCount > 0 || deletable.length > 0) && (
            <section
              aria-label="Branches"
              className="flex flex-col gap-2 rounded-lg bg-card p-4 ring-1 ring-white/12"
            >
      ...
      <ConfirmDialog
        ...
        title="Delete this branch on GitLab?"
        confirmLabel="Delete branch"
        busyLabel="Deleting…"
        busy={del.isPending}
        onConfirm={() => {
          if (confirm?.kind === "branch") del.mutate(confirm.name);
          setConfirm(null);
        }}
      >
        <p>
      ...
      <ConfirmDialog
        ...
        destructive={false}
        busy={retarget.isPending}
        onConfirm={() => {
          if (confirm?.kind === "retarget") retarget.mutate(confirm.id);
          setConfirm(null);
        }}
      >
        <p>
