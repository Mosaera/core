...
export function MergeConfirm({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent aria-label="Merge this merge request?" className="max-w-md">
        <DialogHeader>
          <DialogTitle>Merge this merge request?</DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-2 px-4 text-[12.5px] leading-relaxed text-muted-foreground">
          {isPending ? (
            <p>Asking GitLab whether this can merge…</p>
          ) : error ? (
            /* A failed read is not permission. Say what happened and offer nothing. */
            <p className="text-amber-600 dark:text-amber-400">
              Couldn&rsquo;t reach GitLab to check whether this can merge, so nothing is offered.
            </p>
          ) : (
            <>
              ...
              <p className="text-[11.5px] text-muted-foreground/80">
                This merges on GitLab and cannot be undone from here.
              </p>
            </>
          )}
        </div>
