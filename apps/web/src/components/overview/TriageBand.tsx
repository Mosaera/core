 ...
 *
 *  Mounted as a subsection of `DecisionBand` (5C) — `hideEmpty` drops the "nothing open" line
 *  when a sibling subsection already has something to say, so the page never shows "quiet" next
 *  to real work. */
export function TriageBand({
  ...
  // Was `buckets.length === 0` alone, which silently dropped the thrash-detector rows below
  // whenever the ladder itself had nothing bucketed — a real "stop retrying" signal, gone.
  if (buckets.length === 0 && stuck.length === 0) {
