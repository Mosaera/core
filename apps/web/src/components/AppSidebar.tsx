...
// The Console group is the ONE front door (5A): "New run" was a second entrance to a raw
// submission form that bypassed onboarding. /run still routes (the escape hatch) — it is just
// not advertised in the rail.
const GLOBAL_NAV = [{ to: "/", end: true, label: "Projects", icon: FolderKanban }];
...
// Phase 9: Changes + Artifacts folded into Delivery, Activity folded into Runs, as an internal
// view switch (`DeliverySection` / `RunsSection`) rather than their own rail entries. Their
// routes still resolve — ProjectDetailPage redirects `/changes`, `/artifacts`, `/activity` to
