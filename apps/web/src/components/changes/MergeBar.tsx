...
export function MergeBar({
  ...
  /** ADR-0112's delivery capability, when known — drives the request noun (S4). */
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          {(state === "merged" || state === "mr-open") && project.mr_url && (
            <Button size="sm" nativeButton={false} render={<a href={project.mr_url} target="_blank" rel="noreferrer" />}>
              {state === "merged" ? `View merged ${nouns.short}` : `View ${nouns.short}`}
            </Button>
          )}
          {state === "no-token" && (
            <Button
          ...
          {(state === "ready" || state === "delivered-unpushed" || state === "blocked") && (
            <Button
              ...
              onClick={onOpenMr}
            >
              {mergeBusy ? "Opening…" : `Open ${nouns.request}`}
            </Button>
          )}
        </div>
      </div>
