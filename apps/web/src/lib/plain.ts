...
/** "structural_claims:c1,c2" / "no_vouch:a;b" → a sentence a human can read. Shared by the
 *  receipt (live gate + durable history) and the run diagnosis card — one translation, not a
 *  second vocabulary invented per surface. */
export function humanizeVouch(vouchedBy: string): string {
  if (vouchedBy.startsWith("structural_claims:")) {
    const ids = vouchedBy.slice("structural_claims:".length).split(",").filter(Boolean);
    return SENTENCES.structuralVouch(ids);
  }
  if (vouchedBy.startsWith("no_vouch:")) {
    const guards = vouchedBy
      .slice("no_vouch:".length)
      .split(";")
      .filter(Boolean)
      .map((g) => g.replace(/_/g, " "));
    return guards.length
      ? `No independent verification — ${guards.join(", ")}`
      : "No independent verification";
  }
  return vouchedBy;
}

/** What the delivery gate's recorded ACTION means, plain. `lib/runs.ts` documents the closed
 *  vocabulary (`deliver | revise | require_human`). */
export const ACTION_PLAIN: Record<string, string> = {
  deliver: "delivered",
  revise: "sent back to revise",
  require_human: "required a human decision",
};

export function actionPlain(action: string): string {
  return ACTION_PLAIN[action] ?? action.replace(/_/g, " ");
}
