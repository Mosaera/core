...
#: Kept for callers that report the floor; the rule itself lives in `passwords.py`.
...
def validate_credentials(username: str, password: str) -> str | None:
    """Return a human error message if the username/password are unacceptable, else None.

    Password rules follow NIST SP 800-63B Rev. 4 and live in `passwords.py`, which explains each
    one. The headline: 15 characters because a password is the ONLY authenticator here, and NO
    composition rules — Rev. 4 prohibits them rather than merely discouraging them, because
    "one uppercase, one digit, one symbol" is what produces `Password1!`.
    """
    from mosaera_api.passwords import problem_with

    return username_problem(username) or problem_with(password, username)


def username_problem(username: str) -> str | None:
    """Whether the USERNAME alone is acceptable.

    Its own function because the wizard validates one field at a time, and did it by calling
    `validate_credentials(name, "placeholder8")` — a fake password chosen to satisfy the length
    rule of the day. It stopped satisfying it the moment that rule changed, and the username step
    began rejecting every name. A check that needs a dummy value for the thing it is not checking
    is a check that has been asked the wrong question.
    """
...
#: What `MOSAERA_COOKIE_SECURE` accepts. Split out so `guard_bind` can refuse a value that is
#: NEITHER — red-team round 3 found `on` and `secure` passing the #124 clause and silently meaning
#: OFF, which is the declaration control accepting an answer it cannot read.
COOKIE_SECURE_TRUE = ("1", "true", "yes")
COOKIE_SECURE_FALSE = ("0", "false", "no")
...
def cookie_secure() -> bool:
    ...
    return os.environ.get("MOSAERA_COOKIE_SECURE", "0").strip().lower() in COOKIE_SECURE_TRUE
