...
class RunSessionBase:
    ...
    def __init__(
        ...
        # What this run STARTED with — see runner/_provenance.py for why both are captured
        # once and never re-read.
        self.controls: dict[str, bool] = run_controls(_settings)
        self.profiles: dict[str, str] = run_profiles(_settings.home)
    ...
    def _emit(self, type_: str, data: Any) -> None:
        ...
        with self._events_lock:
            # Stream cursor (SSE `id:`): 1-based position in this session's history. The SSE
            # route sends it as the frame's `id:` line, so a browser's automatic reconnect
            # resends it as `Last-Event-ID` and `_subscribe(after=...)` (_lifecycle.py) replays
            # only what came after — a reconnect resumes instead of duplicating the transcript.
            event["sid"] = len(self._history) + 1
