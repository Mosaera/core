...
class LifecycleMixin(RunSessionBase):
    ...
    def _subscribe(self) -> queue.Queue[dict[str, Any]]:
        ...
        viewer (tabs, a reconnect) gets the full stream from the start."""
        sub: queue.Queue[dict[str, Any]] = queue.Queue()
        with self._events_lock:
            for event in self._history:  # replay backlog to this subscriber
                sub.put(event)
    ...
    async def aevents(self) -> AsyncIterator[dict[str, Any]]:
