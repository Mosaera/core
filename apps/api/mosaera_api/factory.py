...
def default_graph_factory(
    ...
    if req.project_id:
        ...
        if not resume:
            # Base-drift check (ADR-0102 slice D; F11 readiness-review finding), before
            # any branch cut and on EVERY fresh run against the project clone — item-scoped
            # or project-level. The check used to be nested under `if item_branch:`, so a
            # project-level run (no item_id) or the FIRST item run after a squash-merge
            # skipped it entirely and hit the same wrong-diff problem the item-run check
            # exists to prevent, just further downstream. `resume` never re-cuts the clone
            # (the coder's uncommitted work at the gate must survive untouched), so it is
            # excluded here exactly as it always was for the item path.
            #
            # Diverged fails the launch closed — the caller surfaces the reason (HTTP 400 /
            # project.error). Unreachable proceeds: a correctness aid must not break offline
            # or local-dir use, but the skip is recorded.
