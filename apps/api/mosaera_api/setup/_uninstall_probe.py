...
OUR_ENV_KEYS = (
    ...
    "MOSAERA_API_PORT",
    "MOSAERA_API_TOKEN",
    # Minted by the wizard (ADR-0126), so by the rule above it must be removable. It is also the
    # one key here whose removal can cost DATA rather than configuration: it decrypts the stored
    # GitLab PAT, OAuth client secret, App private key and provider keys, and ADR-0039 names
    # losing it as exactly the cost of at-rest encryption. `survives()` warns when this is taken
    # while the database volume is kept, which is the combination that strands them.
    "MOSAERA_SECRET_KEY",
    "MOSAERA_COOKIE_SECURE",
)
