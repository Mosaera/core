...
def build_missing_images(app: SetupApp) -> list[str]:
    """Build whatever is missing, one row at a time, cancellable throughout. Returns the failures.

    NO LONGER A STEP OF ITS OWN. It was one screen out of five, and every one of those screens is a
    place an operator has to understand something — but there is nothing here for them to decide:
    the images are either present or they are not, and the answer is always "build them". It now
    runs inside the single startup step, alongside the dashboard build and the server launch, which
    is the one place where the wizard does WORK rather than asking questions.
...
def build_images_only(app: SetupApp) -> None:
    """The image build reached from the configured screen, which owns landing back on it.

    `build_missing_images` returns failures and paints rows; it does not decide where the operator
    ends up, because inside the startup step the answer is "carry on to the dashboard" and here it
    is "back to the screen that sent me". A shared helper that also navigated would have to know
    which caller it had, which is how one function becomes two behaviours wearing one name.
    """
    failed = build_missing_images(app)
    if not failed and not app._cancel:
        app.call_from_thread(app._note, "Sandbox images are built.")
    from mosaera_api.setup import enter_steps  # local: it imports this module

    app.call_from_thread(enter_steps.reprobe, app)
