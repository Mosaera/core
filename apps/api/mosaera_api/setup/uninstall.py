...
from mosaera_api.setup.ui import DONE, FAILED, RUNNING
...
def plan(settings: Settings, home: Path, repo_root: Path | None = None) -> list[Removable]:
    ...
    # `our_pid` alone means "we wrote a pid file naming a live process". An orphan has no pid file
    # — it was deleted with the installation that wrote it — so the row that stops it would not be
    # OFFERED, and an uninstall with every box ticked still left a server holding the port. Ask the
    # port as well, and only for processes that identify as a Mosaera API.
    from mosaera_api.setup.launch import _our_holders, our_pid

    if our_pid(home, repo_root) or _our_holders(home, repo_root):
        out.append(
            Removable(
                key="server",
...
def _still_there(key: str, docker_bin: str, project: str, repo_root: Path | None = None) -> str:
    """What the removal claimed to take away and did not. "" when it is genuinely gone.

    THREE outcomes, not two. A non-zero exit here means the CHECK could not run — a daemon that
    stopped, a Colima VM too slow for the 15s timeout — and returning "" for that reported the
    volume as removed on the strength of a look that never happened. The command's own failure is
    caught by the caller; this is about the verification failing, which is the quieter case and
    the one that makes an uninstall claim a clean machine it never confirmed.
    """
    if key == "server":
        # Verify the PORT is free, not that a signal was sent. `stop` already escalates
        # SIGTERM -> SIGKILL and reports the process's state; this is the independent check that
        # the thing the operator actually cares about — a bound port — was released.
        from mosaera_api.setup.launch import _our_holders

        left = _our_holders(repo_root or Path.cwd(), repo_root)
        return f"a Mosaera server is still listening (pid {left[0]})" if left else ""
    if key == "containers":
        code, out = _docker(
            docker_bin, "ps", "-aq", "--filter", f"label=com.docker.compose.project={project}"
        )
        if code != 0:
            return f"could not verify the container is gone — Docker did not answer ({project})"
        return f"the container is still there (project {project})" if out.strip() else ""
    if key == "data":
        code, out = _docker(
            docker_bin,
            "volume",
        ...
        if code != 0:
            return f"could not verify the volume is gone — Docker did not answer ({project})"
        return f"the volume is still there (project {project})" if out.strip() else ""
    return ""
...
def perform(
    ...
    for n, item in enumerate(selected):
        ...
        if not failed and (left := _still_there(item.key, settings.docker_bin, project, repo_root)):
