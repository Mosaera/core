...
async def images(app: SetupApp) -> None:
    found = survey_images(app.settings)
    absent = [i for i in found if not i.present]
    if not absent:
        await app._skip()
        return
    app._paint(screens.images(len(absent), len(found)))
...
async def access(app: SetupApp) -> None:
    ...
    app._paint(
        screens.access(
            ...
            blocked=public_bind_blocked_by(app.settings),
...
async def dispatch(app: SetupApp, step: str) -> None:
    ...
    await {
        ...
        "uninstall_confirm": lambda: uninstall_flow.confirm(app),
