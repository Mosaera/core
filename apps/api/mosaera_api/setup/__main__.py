...
def main(argv: list[str] | None = None) -> int:
    ...
    _print_outcome(app, code, repo_root)
    _finalize(code)  # never returns — see below


def _finalize(code: int) -> NoReturn:
    """End the process now, without waiting on a lingering worker thread.

    A `@work(thread=True)` probe or build can still be blocked in a syscall when the countdown
    exits the app: Textual cannot cancel a running thread, and the interpreter's atexit join on
    the worker executor would then hang — AFTER the outcome is printed — so the operator's
    terminal never comes back (reproduced on CachyOS 2026-09-03; the URL printed, the shell never
    returned). By this point the detached server is already up and independent and the wizard's
    visible work is done, so the worker's result no longer matters. `os._exit` skips the atexit
    join (and every finalizer), so flush the outcome first. This mirrors the removal path, which
    `os.execv`'s away for the same "atexit will not run here" reason.
    """
    sys.stdout.flush()
    sys.stderr.flush()
    os._exit(code)
...
def outcome_lines(
    *,
    ...
    """The plain-text record of where setup ended, as a pure function of the facts.

    Separate from the printing for the same reason `screens.py` is separate from `app.py`: what
    the wizard SAYS on its way out can be asserted without a terminal. Every claim here must be
    true for the state being reported — an unknown value is named as unknown, never guessed at.
    """
    lines = [""]
    if removed:
        # The install itself was kept, or `_hand_off_removal` would have exec'd before this ran.
        lines += ["The selected Mosaera components were removed; the installation was kept."]
    elif code == 0 and serving:
        lines += [f"Mosaera is running at  {url}"]
        lines += (
            [f"Sign in as  {username}"]
            if username
            else [
                "No account was created in this session — sign in with the one made",
                "when this instance was first set up.",
            ]
        )
    elif code == 0:
        lines += [
            f"Setup finished, but nothing is answering at {url} right now.",
            "Start the server with  make up  from the install directory.",
        ]
    else:
        # `resume` records the step and greets the next run with it; this names the same fact in
        # the scrollback, because the alternate screen the wizard drew on is gone by now.
        from mosaera_api.setup.resume import _NAMES

        lines += [
            f"Setup did not finish — it stopped at {_NAMES.get(step, f'the {step!r} step')}.",
            "What was already done is saved; running setup again skips it.",
        ]
    if log_exists:
        lines += [f"Server log:  {log}"]
    elif not removed and (code != 0 or not serving):
        lines += ["No server log was written — the server was never started."]
    lines += ["Run setup again at any time with:", f"    {reenter}", ""]
    return lines


def _print_outcome(app: Any, code: int, repo_root: Path) -> None:
    """Say where setup ended, AFTER Textual has restored the terminal.

    The wizard draws on the alternate screen, so everything it ever showed vanishes the moment it
    exits — and the finished screen closes ITSELF after `done_flow.COUNTDOWN_SECONDS`. Before
    this, the address, the account name and the log path all left with it, and an operator who
    looked away for a minute came back to an empty prompt (G4). Whether it answers is asked of
    the port here and now (`responds_ok`), not of what the wizard believed a screen ago.
    """
    from mosaera_api.setup import done_flow, launch
    from mosaera_api.setup.app import REMOVED_STEP
    from mosaera_api.setup.steps import lan_address

    host, port = done_flow.bind(app)
    log = Path(app.settings.home) / launch.LOG_NAME
    for line in outcome_lines(
        ...
        step=app.step,
        removed=app.step == REMOVED_STEP,
        reenter=f"cd {shlex.quote(str(repo_root))} && uv run mosaera-setup",
    ):
        print(line)
