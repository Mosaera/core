...
STEPS = ("welcome", "machine", "images", "database", "access", "admin", "done")
...
class SetupApp(App[int]):
    ...
    def _probe_worker(self) -> None:
        workers.guarded(
            self,
            lambda: self.call_from_thread(self._probed_ready, enter_steps.configured(self)),
            "welcome",
        )

    def _probed_ready(self, ready: enter_steps.Ready | None) -> None:
        enter_steps.probed(self, ready)
    ...
    async def _build_images(self) -> None:
        self._begin("Sandbox images", "Building. Esc cancels.")
        self._build_worker()
    ...
    def _build_worker(self) -> None:
        workers.guarded(self, lambda: build_flow.build_images(self), "images")

    @work(thread=True, exit_on_error=False)
    def _database_worker(self) -> None:
        workers.guarded(self, lambda: build_flow.bundled_database(self), "database")

    @work(thread=True, exit_on_error=False)
    def _uninstall_worker(self) -> None:
        workers.guarded(self, lambda: uninstall_flow.work(self), "uninstall")

    @work(thread=True, exit_on_error=False)
    def _launch_worker(self) -> None:
        workers.guarded(self, lambda: done_flow.bring_up(self), "done")
    ...
    async def _set_access(self, *, public: bool) -> None:
        await choices.set_access(self, public=public)
    ...
    async def _submit_field(self) -> None:
        ...
        if self._field_for == "db_url":
            await choices.submit_db_url(self, value)
            return
        if self._field_for == "db_port":
            await choices.submit_db_port(self, value)
            return
