...
async def submit(app: object, field: str, value: str) -> None:
    ...
    if field == "username":
        if err := validate_credentials(value.strip(), "placeholder8"):
            app._note(err, error=True)  # type: ignore[attr-defined]
            return
        app._username = value.strip()  # type: ignore[attr-defined]
        app._ask("Password", secret=True, for_field="password", hint="At least 8 characters.")  # type: ignore[attr-defined]
        return
