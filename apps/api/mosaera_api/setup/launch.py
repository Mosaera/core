...
def dashboard_built(repo_root: Path) -> bool:
    """Whether the SPA the API serves exists. Without it the link opens a 404, which is a worse
    outcome than saying the build is still to do."""
    return (repo_root / "apps" / "web" / "dist" / "index.html").is_file()
...
def _is_our_server(pid: int, repo_root: Path | None) -> bool:
    ...
    Read from `/proc`, which cannot be forged by whoever wrote the pid file. A kernel without
    `/proc` (or a platform without it) fails CLOSED: unverifiable is not ours.
    """
    try:
        cmdline = Path(f"/proc/{pid}/cmdline").read_bytes().replace(b"\0", b" ").decode()
    except OSError:
        return False
    if "mosaera-api" not in cmdline:
        return False
...
def stop(home: Path, repo_root: Path | None = None) -> str:
    ...
    if not pid:
        return ""
    try:
        os.kill(pid, 15)  # SIGTERM — uvicorn's own shutdown, not a kill
