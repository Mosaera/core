...
def make_backlog_router(ctx: AppContext) -> APIRouter:
    ...
    def run_backlog_item(
        ...
        try:
            ...
            if str(exc).startswith("base drift:"):
                # The autonomous sweep records drift on project.error (advance_project's generic
                # handler), which is the note DriftRecovery renders from. A manual launch hits the
                # same check but used to surface only this HTTP detail — so the recovery action
                # was unreachable exactly when a person was watching. Record it the same way.
                mem.update_project(project_id, error=str(exc))
