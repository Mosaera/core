...
def make_runs_router(ctx: AppContext) -> APIRouter:
    ...
    async def stream_events(run_id: str, request: Request) -> StreamingResponse:
        ...
        session = ctx.get_session(run_id)
        # D4 resume cursor: a browser's automatic EventSource reconnect resends the last frame's
        # `id:` line as `Last-Event-ID` — honor it so the reconnect resumes past what this
        # connection already delivered instead of replaying the whole history into a transcript
        # that never resets client-side. `?after=` covers a deliberate (non-native) resume.
        cursor = request.headers.get("last-event-id") or request.query_params.get("after")
        try:
        ...
        async def agen() -> AsyncIterator[str]:
            async for event in session.aevents(after=after):
                if event["type"] == "_ping":
                    # A bare comment line: valid SSE, ignored by EventSource, never
                    # dispatched as a client event and never a resume cursor.
                    yield ": keep-alive\n\n"
                    continue
                sid = event.get("sid")
    ...
    def set_run_mode(run_id: str, body: dict[str, str]) -> dict[str, Any]:
        ...
        D3 (#116): ``writes_auto`` is only consulted BEFORE a gate raises
        (``_loop.py``), so flipping ask->auto in front of a gate that is ALREADY
        parked does nothing to it — the switch takes hold at the run's next write.
        Rather than silently doing nothing (or reaching into approval semantics to
        auto-resolve a park a manual approval didn't touch — off limits here), the
        response says so plainly so the UI can render the honest pending-vs-effective
        state instead of implying the open gate was just resolved."""
        ...
        currently_parked = session.status == "awaiting_approval"
        return {
            "run_id": run_id,
            "mode": str(body.get("mode")),
            "previous": previous,
            "currently_parked": currently_parked,
            "effective": "next_gate" if currently_parked else "immediate",
        }
