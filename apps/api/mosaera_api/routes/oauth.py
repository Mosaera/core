...
def _audit_project(ctx: AppContext, project_id: str, actor: str, event: str, detail: str) -> None:
    """Best-effort audit against the project's newest run — the same anchor `_audit_connect`
    (this module) and `github_connect._audit` use. Never fails the request."""
    ...
    try:
        ...
        if runs:
            ctx.history.add_audit_event(str(runs[0]["id"]), event, f"actor={actor}; {detail}")
    except Exception:  # noqa: S110 — audit is best-effort, never blocks the request
        pass
...
def make_oauth_router(ctx: AppContext, require_admin: Callable[[Request], None]) -> APIRouter:
    ...
    @api.post("/projects/{project_id}/gitlab/recheck")
    def gitlab_recheck(project_id: str, request: Request) -> dict[str, object]:
        """A per-project liveness probe (task 4B-i, F1/F2): a stored `has_gitlab_token` bit is
        presence, not proof — a revoked or expired PAT still reads as connected until something
        actually asks GitLab. This spends ONE cheap authenticated read (`get_user`, the same call
        the instance-level status card already makes) with the project's OWN token, never a
        global one. Mirrors `github_connect`'s per-project Connect, which is the one place in
        this codebase that already re-resolves rather than trusting a cached bit.
        """
        ...
        if not token:
            return {"ok": False, "verified": False, "error": "this project has no GitLab token"}
        user, err = glc.get_user(settings.gitlab_url, token)
        if err or not isinstance(user, dict):
            return {"ok": True, "verified": False, "error": err or "unexpected response"}
        return {"ok": True, "verified": True, "error": None}

    @api.post("/projects/{project_id}/gitlab/disconnect")
    def gitlab_disconnect(project_id: str, request: Request) -> dict[str, object]:
        """Clear this project's GitLab token pair (F3 — no disconnect existed for it). The
        credential-write tier, same as every other project token mutation."""
        require_admin(request)
        mem = ctx.require_memory()
        detail = mem.project_detail(project_id)
        if detail is None:
            raise HTTPException(status_code=404, detail="unknown project")
        mem.update_project(project_id, gitlab_token="", gitlab_api_token="")
        user = current_user(request, ctx.history)
        actor = f"user:{user['username']}" if user and user.get("username") else "endpoint"
        _audit_project(ctx, project_id, actor, "project.gitlab_disconnected", "token pair cleared")
