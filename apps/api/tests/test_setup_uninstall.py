...
def test_the_confirm_screen_starts_on_cancel_and_cancelling_removes_nothing(tmp_path: Path) -> None:
    """The gate that stands between ticking "delete all project data" and it happening.
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            await uninstall_flow.confirm(app)
            await pilot.pause()
...
def test_every_selected_item_reports_progress(tmp_path: Path) -> None:
...
def test_every_door_into_the_picker_arrives_empty(tmp_path: Path) -> None:
    """Same screen, same words — it used to arrive armed or empty depending on which of three doors
    you came through: Ctrl-X and the configured screen pre-ticked the reversible rows, the finished
    screen ticked nothing."""
    import asyncio
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            # Door 1: the menu path (`enter`).
            app.step = "done"
            await uninstall_flow.enter(app)
            assert app._chosen == set(), "the menu door arrived with rows ticked"

            # Door 2: the abandon chord, from a step in the middle of the flow.
            app.step = "database"
            await uninstall_flow.abandon(app)
            assert app._chosen == set(), "Ctrl-X arrived with rows ticked"
            assert app._returns_to == "database", "cancelling would not return where it came from"

    asyncio.run(_body())


def test_returning_to_the_picker_keeps_what_was_ticked(tmp_path: Path) -> None:
    """Arriving starts empty; RETURNING keeps your ticks. The clearing used to live in `enter`,
    which a re-entry also calls — so the Cancel row had to route around it by hand, and the
    "Nothing is selected" bounce would have wiped a selection had there been one."""
    import asyncio
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            app.step = "done"
            await uninstall_flow.enter(app)
            uninstall_flow.toggle(app, 0)
            uninstall_flow.toggle(app, 1)
            assert app._chosen == {0, 1}

            # Cancel from the confirm screen.
            app.step = "uninstall_confirm"
            await uninstall_flow.confirmed(app, 0)
            assert app._chosen == {0, 1}, "cancelling discarded the selection"

            # And the "Nothing is selected" bounce, which is also a return.
            app._chosen = set()
            await uninstall_flow.choose(app, len(app._removable))
            assert "Nothing is selected" in app._notice
            uninstall_flow.toggle(app, 0)
            await uninstall_flow.choose(app, len(app._removable))
