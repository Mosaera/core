...
def test_diverged_base_fails_a_project_level_run_too(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """F11 (readiness review): the drift check used to run only when an item branch was cut,
    so a project-level run (no item_id) — and the first item run after a squash-merge,
    before an item_id existed to gate it — skipped the check entirely. It must now fire for
    every fresh (non-resume) run against the project clone."""
    import mosaera_api.factory as factory_mod
    from mosaera_api.factory import default_graph_factory
    from mosaera_api.schemas import RunSubmit
    from mosaera_core.tools.repo import DriftStatus
    ...
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    req = RunSubmit(repo="unused", task="t", project_id="p1")  # no item_id: project-level

    seen: list[Any] = []

    def _check(root: Any, *a: Any, **k: Any) -> DriftStatus:
        seen.append(root)
        return DriftStatus("diverged", "split history")

    monkeypatch.setattr(factory_mod, "check_base_drift", _check)
    with pytest.raises(RuntimeError, match="base drift: split history"):
        default_graph_factory(req, "r-drift-project")
    assert seen, "the check must actually run for a project-level launch"


def test_diverged_base_does_not_block_a_resume(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    """A resume never re-cuts the clone — the coder's uncommitted work at the gate must survive
    untouched — so the drift check (which can hard-reset on fast-forward) must not run for it."""
    import mosaera_api.factory as factory_mod
    from mosaera_api.factory import default_graph_factory
    from mosaera_api.schemas import RunSubmit
    from mosaera_core.tools.repo import DriftStatus
    ...
    def _boom(*a: Any, **k: Any) -> DriftStatus:  # pragma: no cover - must never run
        raise AssertionError("resume must never run the drift check")

    monkeypatch.setattr(factory_mod, "check_base_drift", _boom)
    with pytest.raises(FileNotFoundError):  # proceeds straight to reopening the (missing) clone
        default_graph_factory(req, "r-drift-resume", resume=True)
...
# --- clone reset (task 4D, F12: the diverged-recovery action) -----------------------------


def _clone_reset_mem(prior_error: str = "") -> Any:
    class _Mem(_FakeMemoryWithDiff):
        def __init__(self) -> None:
            super().__init__()
            self.updates: list[dict[str, Any]] = []
            self.audits: list[tuple[str, str, str]] = []
            self._error = prior_error

        def project_detail(self, pid: str) -> dict[str, Any] | None:
            if pid != "p1":
                return None
            return {"id": "p1", "error": self._error, "runs": [{"id": "run-1"}]}
...
def _seed_clone_with_divergence(tmp_path: Any) -> None:
    """A real local remote + a persistent project clone that has diverged from it — the exact
    shape `check_base_drift` would report `diverged` for."""
    from git import Repo
    from mosaera_core.tools.repo import clone_project

    remote = tmp_path / "remote"
    remote.mkdir()
    rrepo = Repo.init(remote, initial_branch="main")
    with rrepo.config_writer() as cw:
        cw.set_value("user", "name", "T")
        cw.set_value("user", "email", "t@local")
    (remote / "a.txt").write_text("one\n", encoding="utf-8")
    rrepo.index.add(["a.txt"])
    rrepo.index.commit("add a")
    ...
    (remote / "b.txt").write_text("remote-side\n", encoding="utf-8")
    rrepo.index.add(["b.txt"])
    rrepo.index.commit("add b (remote)")

    (ws.root / "c.txt").write_text("local-side\n", encoding="utf-8")
    ws.repo.index.add(["c.txt"])
    ws.repo.index.commit("add c (local)")


def test_reset_endpoint_forces_the_clone_and_clears_the_drift_note(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    from git import Repo
    ...
    monkeypatch.setenv("MOSAERA_ALLOW_REMOTE_CONFIG", "1")
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    _seed_clone_with_divergence(tmp_path)

    mem = _clone_reset_mem(prior_error="base drift: origin/main and the local tip diverge")
    r = _client_with(mem).post("/api/projects/p1/clone/reset")
    assert r.status_code == 200
    assert r.json()["reset"] is True

    clone_repo = Repo(tmp_path / "projects" / "p1" / "repo")
    remote_repo = Repo(tmp_path / "remote")
    assert clone_repo.head.commit.hexsha == remote_repo.head.commit.hexsha
    assert not (tmp_path / "projects" / "p1" / "repo" / "c.txt").exists()

    assert {"id": "p1", "error": ""} in mem.updates, "the drift pause note must be cleared"
    assert any(e == "clone.reset" for _run, e, _d in mem.audits)


def test_reset_endpoint_leaves_an_unrelated_error_alone(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    monkeypatch.setenv("MOSAERA_ALLOW_REMOTE_CONFIG", "1")
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    ...
    mem = _clone_reset_mem(prior_error="budget exceeded")
    r = _client_with(mem).post("/api/projects/p1/clone/reset")
    assert r.status_code == 200
    assert {"id": "p1", "error": ""} not in mem.updates, (
        "only a drift note is this endpoint's to clear"
    )


def test_reset_endpoint_refuses_a_non_admin_session(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    monkeypatch.setenv("MOSAERA_ALLOW_REMOTE_CONFIG", "1")
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    ...
    class _NonAdmin(type(_clone_reset_mem())):  # type: ignore[misc]
    ...
    r = client.post("/api/projects/p1/clone/reset")
    assert r.status_code == 403


def test_reset_endpoint_refuses_an_unknown_project(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    monkeypatch.setenv("MOSAERA_ALLOW_REMOTE_CONFIG", "1")
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    r = _client_with(_clone_reset_mem()).post("/api/projects/nope/clone/reset")
    assert r.status_code == 404


def test_reset_endpoint_502s_when_the_remote_is_unreachable(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Any
) -> None:
    from git import Repo
    ...
    monkeypatch.setenv("MOSAERA_HOME", str(tmp_path))
    _seed_clone_with_divergence(tmp_path)
    Repo(tmp_path / "projects" / "p1" / "repo").remotes.origin.set_url(str(tmp_path / "gone"))

    r = _client_with(_clone_reset_mem()).post("/api/projects/p1/clone/reset")
    assert r.status_code == 502


def test_manual_launch_drift_writes_the_recovery_note(monkeypatch: pytest.MonkeyPatch) -> None:
    """A guided/manual launch that hits base drift must leave the same `project.error` note the
    autonomous sweep leaves — DriftRecovery renders from that note, and before this the manual
    path surfaced only a transient 400, so the recovery action was unreachable exactly when a
    person was watching (wave-1 disclosure)."""
    from mosaera_api.routes.context import AppContext

    mem = _clone_reset_mem()

    def _detail(pid: str) -> dict[str, Any] | None:
        if pid != "p1":
            return None
        return {"id": "p1", "error": "", "runs": [], "default_run_mode": "guided"}

    mem.project_detail = _detail  # type: ignore[method-assign]
    mem.get_backlog_item = lambda iid: {"id": iid, "project_id": "p1", "title": "t"}  # type: ignore[attr-defined]

    monkeypatch.setattr("mosaera_api.routes.backlog.guard_can_run", lambda: None)

    def _drift(self: Any, *a: Any, **k: Any) -> Any:
        raise RuntimeError("base drift: origin/main and the local tip diverge")

    monkeypatch.setattr(AppContext, "launch_item", _drift)

    r = _client_with(mem).post("/api/projects/p1/backlog/7/run")
    assert r.status_code == 409
    assert r.json()["detail"].startswith("base drift:")
    assert any(
        u.get("id") == "p1" and str(u.get("error", "")).startswith("base drift:")
        for u in mem.updates
    ), "the drift note must be recorded so DriftRecovery can render it"


def test_manual_launch_other_failures_do_not_write_the_note(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The note is for drift only — an unrelated startup failure stays a 400 and writes nothing,
    so the recovery button cannot appear over a problem it cannot fix."""
    from mosaera_api.routes.context import AppContext
    ...
    def _boom(self: Any, *a: Any, **k: Any) -> Any:
        raise RuntimeError("clone exploded for some other reason")

    monkeypatch.setattr(AppContext, "launch_item", _boom)
