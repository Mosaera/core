...
import pytest
from mosaera_api.setup import launch, resume, screens
...
def test_ctrl_x_reaches_the_picker_from_mid_flow_and_returns_where_it_came_from(
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            assert app._chosen == set(), "a destructive list must not arrive pre-armed"
...
def test_space_toggles_a_row_and_the_action_rows_are_not_toggleable(tmp_path: Path) -> None:
    from mosaera_api.setup import uninstall_flow

    # A removable has to actually EXIST to be toggled. The list is gated on what happened now, so
    # this writes one of the wizard's own `.env` keys rather than relying on rows that used to be
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.pause()
            assert app._chosen == set()
            await pilot.press("space")
            await pilot.pause()
            assert app._chosen == {0}
            await pilot.press("space")
            await pilot.pause()
            assert app._chosen == set()
            # The two trailing rows are actions; arming one like an item would make the list
            # ambiguous about what Enter does.
            uninstall_flow.toggle(app, len(app._removable))
            assert app._chosen == set()
...
def test_a_worker_that_raises_neither_kills_the_app_nor_strands_it(
    ...
    def _boom(_app: Any) -> None:
        raise RuntimeError("disk full")

    monkeypatch.setattr("mosaera_api.setup.build_flow.build_images", _boom)

    async def _body() -> None:
        app = SetupApp(tmp_path)
        async with app.run_test() as pilot:
            app.step = "images"
            app._begin("Sandbox images", "…")
            app._build_worker()
            await app.workers.wait_for_complete()
            await pilot.pause()
...
def test_the_data_row_does_not_promise_to_delete_a_database_it_cannot_reach() -> None:
    ...
    row = next(r for r in plan(external, Path("/nonexistent")) if r.key == "data")
    assert "external" in row.detail

    bundled = dc.replace(Settings.from_env(), db_url="postgresql://u:p@localhost:5432/mosaera")
    row = next(r for r in plan(bundled, Path("/nonexistent")) if r.key == "data")
...
def test_the_configured_screen_names_what_is_outstanding() -> None:
    ...
    assert "Runs are refused" in gapped.body
...
def test_the_configured_screen_never_offers_to_leave_running_a_stopped_instance() -> None:
    ...
    assert up.choices[0] == "Leave it running"
    ...
    assert up.choices[1:] == down.choices[1:] == ["Re-run setup", "Uninstall Mosaera"]


def test_the_first_row_starts_a_stopped_instance_and_leaves_a_running_one(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from mosaera_api.setup import choices
    ...
    async def _body() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            # Running: the first row changes nothing at all.
            monkeypatch.setattr(launch, "already_serving", lambda *_a, **_k: True)
            _STARTED.clear()
            app.step = "configured"
            await choices.dispatch(app, 0)
...
# --------------------------------------------------------- an open port is not a working instance
