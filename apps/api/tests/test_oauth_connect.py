...
class _OAuthMem:
    ...
    def __init__(self, session: dict[str, Any] | None, *, project_token: str | None = None) -> None:
...
# ---- per-project recheck / disconnect (task 4B, F1/F2/F3) --------------------------------


def test_recheck_spends_the_projects_own_token_and_reports_verified(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from mosaera_api.routes import oauth as oauth_mod

    seen: list[tuple[str, str]] = []

    def _get_user(url: str, token: str) -> tuple[dict[str, Any], None]:
        seen.append((url, token))
        return {"username": "bot"}, None

    monkeypatch.setattr(oauth_mod.glc, "get_user", _get_user)
    mem = _OAuthMem(_ADMIN, project_token="glpat-real")
    r = _client(mem).post("/api/projects/p1/gitlab/recheck")
    assert r.status_code == 200
    assert r.json() == {"ok": True, "verified": True, "error": None}
    assert seen == [("https://gitlab.rengifo.me", "glpat-real")]


def test_recheck_reports_a_rejected_credential_honestly(monkeypatch: pytest.MonkeyPatch) -> None:
    from mosaera_api.routes import oauth as oauth_mod

    monkeypatch.setattr(oauth_mod.glc, "get_user", lambda url, token: (None, "401: unauthorized"))
    mem = _OAuthMem(_ADMIN, project_token="glpat-revoked")
    r = _client(mem).post("/api/projects/p1/gitlab/recheck")
    assert r.status_code == 200
    body = r.json()
    assert body == {"ok": True, "verified": False, "error": "401: unauthorized"}


def test_recheck_without_a_token_says_so_without_spending_a_call(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from mosaera_api.routes import oauth as oauth_mod

    def _boom(*a: Any, **k: Any) -> Any:  # pragma: no cover - must never run
        raise AssertionError("no token means nothing is askable")

    monkeypatch.setattr(oauth_mod.glc, "get_user", _boom)
    mem = _OAuthMem(_ADMIN, project_token=None)
    r = _client(mem).post("/api/projects/p1/gitlab/recheck")
    assert r.status_code == 200
    assert r.json() == {"ok": False, "verified": False, "error": "this project has no GitLab token"}


def test_recheck_refuses_a_non_admin_session() -> None:
    mem = _OAuthMem({"id": 7, "username": "user", "is_admin": False}, project_token="glpat-x")
    assert _client(mem).post("/api/projects/p1/gitlab/recheck").status_code == 403


def test_disconnect_clears_the_token_pair_and_audits() -> None:
    mem = _OAuthMem(_ADMIN, project_token="glpat-x")
    r = _client(mem).post("/api/projects/p1/gitlab/disconnect")
    assert r.status_code == 200
    assert r.json() == {"disconnected": True}
    assert {"id": "p1", "gitlab_token": "", "gitlab_api_token": ""} in mem.updates
    assert any(e == "project.gitlab_disconnected" for _run, e, _d in mem.audits)


def test_disconnect_refuses_a_non_admin_session() -> None:
    mem = _OAuthMem({"id": 7, "username": "user", "is_admin": False}, project_token="glpat-x")
    r = _client(mem).post("/api/projects/p1/gitlab/disconnect")
    assert r.status_code == 403
    assert mem.updates == [], "nothing may be cleared on a refused disconnect"


def test_disconnect_refuses_an_unknown_project() -> None:
    mem = _OAuthMem(_ADMIN)
    r = _client(mem).post("/api/projects/nope/gitlab/disconnect")
    assert r.status_code == 404
