...
def test_it_states_the_credential_rule_rather_than_just_refusing() -> None:
    out = create_admin(_s(_Store()), "a", "secret123")
    assert not out.ok and "3-64" in out.message
    short = create_admin(_s(_Store()), "alex", "short")
    assert not short.ok and "8 characters" in short.message
...
def test_each_store_refusal_keeps_its_own_cause() -> None:
    ...
    # cannot get past.
    taken = create_admin(_s(_Store(raises=ValueError("username_taken"))), "alex", "secret123")
    assert "taken" in taken.message
    limit = create_admin(_s(_Store(raises=ValueError("user_limit"))), "alex", "secret123")
    assert "account limit" in limit.message


def test_a_dead_database_is_named_not_swallowed() -> None:
    out = create_admin(_s(_Store(raises=RuntimeError("connection refused"))), "alex", "secret123")
