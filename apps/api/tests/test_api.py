...
def test_guard_bind_refuses_unauthenticated_public_bind() -> None:
...
def test_cli_bind_host_closes_the_factory_bypass(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from mosaera_api.app import _cli_bind_host, _host_from_bind
...
def test_providers_test_endpoint(monkeypatch: pytest.MonkeyPatch, tmp_path: Any) -> None:
    ...
    # A local provider isn't testable → 422.
    assert (
        c.post("/api/providers/test", headers=hdr, json={"provider": "ollama"}).status_code == 422
    )

    # A good key → the models it grants.
    monkeypatch.setattr(
        "mosaera_api.routes.settings.fetch_provider_models",
    ...
    monkeypatch.setattr("mosaera_api.routes.settings.fetch_provider_models", _reject)
...
def test_providers_test_endpoint_decrypts_stored_key(
    ...
    monkeypatch.setattr("mosaera_api.routes.settings.fetch_provider_models", _capture)
