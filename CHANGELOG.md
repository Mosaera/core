...
Work landed on `staging`/`main` after the `v0.6.3` tag (`253fa343`). Ships in the next release; no
version number or benchmark claim is made for it here (ADR-0055).

- **Per-user API keys (ADR-0127)** — revocable, attributed, never admin (including behind a reverse
  proxy, `bd5e4dfc`); settings panel; red-teamed 3 rounds / 27 attacks.
- **Core settings are intent profiles (ADR-0122)** — Effort / Quality bar / Independent
  verification, each rendered from the engine's own tables with a differences-only comparison;
  the provably-inert `reviewer_advisory` knob removed; finished runs stamp the profiles they
  started with.
- **Encrypt-at-rest by default (ADR-0126, #123/#124)** — the wizard mints `MOSAERA_SECRET_KEY` on
  every install; an exposed bind without a key or a declared TLS posture refuses to start;
  red-team pass fixed 4 findings.
- **GitLab creates repositories too (ADR-0125)** — same one-grant flow as GitHub, created private,
  finishes credentialed; GitLab gets the GitHub-style setup checklist.
- **Setup wizard hardening sweep** — password confirm + reset, stop-the-instance from the
  configured screen, port pickers instead of diagnoses, uninstall leaves no running server,
  independent residue check, settings applied (not just saved) under a running server,
  content-based sandbox freshness.
- **Installer/distribution** — `install.mosaera.dev` proxies the mirror (with a check that proves
  it), uv install is consented, explicit `exit` so bash stops reading the script it stands in,
  publish refuses a ref behind/ahead of its origin.
- **API** — unknown `/api` paths are 404 for every verb.
- **Alpha productization, waves 2–3 (2026-09-02)** — the product-level pass:
  - *First contact* — one front door ("New run" demoted from the rail); the first screen explains
    Mosaera in user language (no env vars, no internal codenames); a single consolidated "Needs
    you" band replaces five competing attention surfaces; a shared loading/error/retry state
    adopted across the previously silent query components; a real 404 page replaces the silent
    blank on unknown routes; raw engine tokens (vouch strings, receipt chips, park causes)
    translated through the plain-language layer with the raw token in a tooltip.
  - *Navigation* — the project hub goes 9 tabs → 6: Changes and Artifacts fold into Delivery as
    internal views, Activity folds into Runs; old URLs redirect.
  - *Chat honesty (10A)* — `pm_chat_tools` is ON by default (ADR-0111 accepted: the PM chat may
    read its own engine-authored ledgers); starter chips now advertise only what the chat can do.
  - *QA* — a Playwright smoke path (real Chromium + API + throwaway Postgres): login → local-first
    project → Settings→Models → enforced non-blank unknown route → logout; it caught two real
    cross-branch integration bugs (a tsc-only type error and vitest globbing the e2e specs).
  - *Docs* — `docs/user-guide/core-workflow.md` and `docs/user-guide/troubleshooting-runs.md`.
- **Alpha productization, wave 1 (2026-09-02)** — four arcs from the alpha-readiness review:
  - *Setup/installer* — install.sh survives an abandoned wizard (prints the install dir + re-entry
    command instead of dying under `set -e`); the wizard prints its outcome (URL, account, log
    path, re-entry command) after the alternate screen closes, on both exit paths; the
    done/configured screens no longer claim "a run is refused" without a model — they say what
    actually happens; the deleted ADR-0040 setup-token flow is purged from `.env.example`,
    getting-started, and the dashboard guide; real clone URL + Colima truth in the docs.
  - *Models/settings (Ollama first-class)* — `POST /providers/test` gains a local branch that
    probes Ollama for real, which unblocks the Roles table's Test→Save flow for the default
    provider (previously unsavable); ProviderCard's local dot is probed, not asserted; the picker
    distinguishes served from configured-but-absent ("not pulled"), filters embedding models out
    of chat roles, badges keyless hosted models, and gains free-text model entry; a per-check
    health panel (status → explanation → fix, from `/api/preflight`) lands on Settings → Models;
    settings visibility reconciled (vanished groups restored, cannot-fire escalation toggles
    hidden, orphaned `stall_limit` reunited); saves report rejected fields instead of silently
    dropping them, and a NaN can no longer unset a knob; `routes/settings.py` split
    (`routes/providers.py`); dead `presets.py`/preset vocabularies removed.
  - *Guided-run realtime (#116)* — SSE frames carry ids and the stream resumes via
    `Last-Event-ID` instead of replaying the whole transcript on reconnect; an idle park sends
    keep-alive comments so proxies stop killing the quiet stream; the connected indicator
    recovers; `PATCH /runs/{id}/mode` reports whether a switch reached the open gate or only the
    next one, and the workbench says so in front of a parked gate.
  - *Git workflow* — provider-correct vocabulary everywhere (a GitHub user is never told to open
    a "merge request"); MR/PR compose (title/body/target) reaches GitHub on both the create and
    already-open-PR paths instead of being silently discarded; connection health is verified live
    (App JWT probe, per-project GitLab recheck) rather than presence-only; disconnect actions for
    GitHub App / installation / GitLab tokens; read-only installation-repo browsing;
    diverged-clone recovery (`POST /projects/{id}/clone/reset`, guarded) + drift check on
    project-level runs; a failed clone cleans up so retry works; the local-first Delivery page
    links to Publish instead of dead-ending.
