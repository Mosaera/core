...
from mosaera_core.config._profiles import SENSITIVITY_OWNED
...
def test_profile_supplies_a_value_the_operator_never_set() -> None:
    ...
    resolved = layer_knobs({}, {"effort_profile": "persistent"})
...
def test_stored_value_outranks_the_profile() -> None:
    ...
    resolved = layer_knobs({}, {"effort_profile": "persistent", "max_reason_attempts": 0})
...
def test_env_selects_the_profile_itself() -> None:
    ...
    resolved = layer_knobs(
        {"MOSAERA_EFFORT_PROFILE": "cautious"},
        {"effort_profile": "persistent"},
    )
    assert resolved["max_reason_attempts"] == 0
...
def test_unset_or_unknown_choice_derives_nothing() -> None:
    ...
    assert resolve_profiles({"effort_profile": None}) == {}
    assert resolve_profiles({"effort_profile": "nonsense"}) == {}
...
def test_profiles_never_touch_sensitivity_owned_knobs() -> None:
    """The regression guard on a defect that ACTUALLY HAPPENED.

    ``apply_reliability_sensitivity`` runs in ``build_graph`` — after this layer — and overwrites
    the knobs in ``SENSITIVITY_OWNED``. A profile that set one of them was silently replaced:
    ``recovery_profile=persistent`` with ``reliability_sensitivity=cautious`` resolved to
    ``max_escalations=0``, the opposite of what was selected, with nothing telling the operator.
    A control that can be proven not to fire is the whole reason profiles read as theatre.
    """
    for field, table in PROFILE_DERIVED.items():
        for choice, derived in table.items():
            overlap = set(derived) & SENSITIVITY_OWNED
            assert not overlap, (
                f"{field}={choice} derives {sorted(overlap)}, which "
                f"apply_reliability_sensitivity overwrites at graph-build time"
            )


def test_effort_drives_the_sensitivity_dial_rather_than_racing_it() -> None:
    """One dial, not two. `effort_profile` OWNS `reliability_sensitivity`, so the budgets it does
    not set directly are scaled consistently instead of by an independent knob."""
    for choice in ("cautious", "balanced", "persistent"):
        assert PROFILE_DERIVED["effort_profile"][choice]["reliability_sensitivity"] == choice


def test_the_profile_survives_graph_build() -> None:
    """End to end through the seam the defect lived in: select a profile, build the settings the
    graph will actually use, and confirm the selection is still there."""
    from mosaera_core.config import Settings
    from mosaera_core.graph.build import apply_reliability_sensitivity

    env = {"MOSAERA_EFFORT_PROFILE": "persistent", "MOSAERA_HOME": "/nonexistent-probe"}
    built = apply_reliability_sensitivity(Settings.from_env(env))
    # `persistent` grants more rope, and the scaling agrees with the profile instead of undoing it.
    assert built.max_escalations >= 2
    assert built.max_reason_attempts == 3
    assert built.reliability_sensitivity == "persistent"

    cautious = apply_reliability_sensitivity(
        Settings.from_env({**env, "MOSAERA_EFFORT_PROFILE": "cautious"})
    )
    assert cautious.max_escalations == 0
    assert cautious.max_reason_attempts == 0
