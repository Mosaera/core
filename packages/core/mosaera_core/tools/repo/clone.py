...
def _clone_into(
    ...
    try:
        ...
        # task 4E: a failed clone used to leave `dest` behind — usually just the .git
        # directory `Repo.clone_from` half-wrote before the transport error, but a
        # failure inside `_init_empty`/`_prepare_workspace` can leave a full clone with
        # a partially-prepared working tree too. Either way "destination path already
        # exists" then poisoned every retry with the SAME bad source, since nothing ever
        # cleaned it up. `ignore_errors=True`: this is best-effort cleanup on an already-
        # failing path — a second (permissions/locking) error here must not shadow the
        # real one the caller is about to see.
...
@dataclass(frozen=True)
class ResetOutcome:
    """The result of forcing a project clone back onto its remote base (task 4D)."""

    ok: bool
    detail: str = ""


def reset_clone_to_remote(
    root: Path, base: str | None = None, *, timeout: int = 30
) -> ResetOutcome:
    """The diverged-recovery action: fetch ``origin/<base>`` and hard-reset the current branch
    onto it, UNCONDITIONALLY — unlike ``check_base_drift``'s own reset, which only ever
    fast-forwards a tip that is a strict ancestor of the remote. This is the human-directed
    escape hatch for the ``diverged`` case that check refuses to touch on its own: any local-only
    commits (the ones ``check_base_drift`` reported as the other half of the divergence) are
    discarded, on purpose — the whole point of a guarded, typed-confirmation reset (F12).

    Caller holds the project mutex (same discipline as every other clone-mutating route in
    ``project_delivery.py``) — this function does not reserve it itself, so it composes with
    the existing 409-on-active-run pattern rather than duplicating it.
    """
    if not (root / ".git").is_dir():
        return ResetOutcome(False, f"no project clone at {root}")
    repo = Repo(root)
    if not any(r.name == "origin" for r in repo.remotes):
        return ResetOutcome(False, "clone has no origin remote to reset against")
    if base is None:
        from mosaera_core.tools.repo.diff import project_base

        base = project_base(Workspace(root=root, run_id="reset", branch=repo.active_branch.name))
    try:
        repo.git.fetch("origin", base, kill_after_timeout=timeout)
    except GitCommandError as exc:
        return ResetOutcome(False, scrub_credentials(str(exc)))
    try:
        remote_base = repo.commit(f"origin/{base}")
    except Exception:
        return ResetOutcome(False, f"origin/{base} not resolvable after fetch")
    before = repo.head.commit.hexsha[:8] if repo.head.is_valid() else "(empty)"
    repo.git.reset("--hard", f"origin/{base}")
    repo.git.clean("-fd")  # matches open_project_workspace's own run-start sweep
    return ResetOutcome(True, f"{before} → {remote_base.hexsha[:8]}")
