...
_ALLOWED_KEYS = frozenset(
    {
        ...
        # not set them directly. `autonomy_profile`/`recovery_profile` were collapsed into
        # `effort_profile`; a stored value under either old key is dropped on read (unknown keys
        # are filtered), which is the intended outcome — the pair only ever shipped to staging.
        "effort_profile",
        "quality_profile",
