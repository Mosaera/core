...
from mosaera_core.model_sources import ollama_model_list, served_ollama_tags
...
def _ollama_configured_models(settings: Settings) -> list[str]:
    """Ollama-bound role models — the "keep it even if the tag query blips" half of the list."""
    return [
...
def list_models(settings: Settings) -> list[str]:
    ...
    """
    served = served_ollama_tags(settings.ollama_base_url)
    return ollama_model_list(served, _ollama_configured_models(settings))


def list_model_sources(settings: Settings) -> list[dict[str, Any]]:
    """Available models grouped by provider, e.g. ``[{"source": "Ollama", "models": [...],
    "served": [...]}, ...]``. Ollama is discovered live; a hosted provider (BYOM #21)
    contributes its curated suggestions plus any model bound to a role. Empty groups are
    omitted. ``served`` (#119 O1-O3) is the subset of ``models`` CONFIRMED usable right now
    (Ollama tags actually pulled; a hosted provider's live-fetched grant list once a key has
    been tested) — a real, nameable gap the picker can badge instead of presenting every
    listed model as equally ready."""
    served_ollama = served_ollama_tags(settings.ollama_base_url)
    ollama_models = ollama_model_list(served_ollama, _ollama_configured_models(settings))
    groups: list[dict[str, Any]] = [
        {"source": "Ollama", "models": ollama_models, "served": served_ollama}
    ]
    for pid in settings.providers:
        ...
        # `live is not None` means a key was actually put to the provider's list-models endpoint
        # and these ARE the models it granted — a verified suggestion/binding is NOT served until
        # that has happened, however plausible the name looks.
        served = list(live) if live is not None else []
        groups.append({"source": pid.capitalize(), "models": models, "served": served})
