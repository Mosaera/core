...
GENERAL_KNOBS: tuple[Knob, ...] = (
    # INTENT profiles (ADR-0122). The Core surface: the operator states how hard to try, how high
    # the bar is and how much independent checking to demand, and `_profiles.PROFILE_DERIVED` maps
    # that to the mechanics below. They supply DEFAULTS, never ceilings — an explicitly set knob
    # still wins — which is why they can be added to a shipped install without changing behaviour.
    # THREE, not four: `autonomy` and `recovery` both answered "how hard does it try?" and no
    # operator could separate them, so they are one `effort_profile` whose vocabulary matches the
    # `reliability_sensitivity` dial it drives.
    #
    ...
    # server guessed to assemble (ADR-0111). Read-only ledger queries only, over engine-recorded
    # runs/backlog/dependency-edge ledgers — the repository stays out of the chat.
    #
    # ON by default (Phase 10A, 2026-09-02): ADR-0111 is accepted and shipped, and its own
    # security analysis found no new trust boundary — no untrusted bytes, no write path. OFF
    # stays available for the byte-identical single call; costs several model calls per turn.
    Knob("pm_chat_tools", "MOSAERA_PM_CHAT_TOOLS", "bool", True),
...
def coerce_general_patch(patch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Kept for existing callers/tests that want the raising behaviour. The settings route uses
    ``coerce_general_patch_report`` instead (#task-9, honest save semantics) — that variant never
    raises; it reports WHY a field was rejected instead of silently dropping it or 400-ing the
    whole patch for one bad field."""
    applied, rejected = coerce_general_patch_report(patch)
    choice_or_range = next((f for f, why in rejected.items() if "must" in why), None)
    if choice_or_range is not None:
        raise ValueError(f"{choice_or_range} {rejected[choice_or_range]}")
    return applied


def coerce_general_patch_report(patch: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, str]]:
    """Validate a UI settings patch against the knob spec: ``(applied, rejected)``.

    ``applied`` is exactly what ``write_settings`` should persist. ``rejected`` maps a field name
    to a human reason it was NOT applied — an unknown field, a blank/unparsable value, a negative
    number, or a value outside its declared ``choices``. Never raises: the whole patch is processed
    field-by-field so one bad field cannot silently swallow (or fully block) the rest — the caller
    reports both halves honestly instead of a blanket "Saved" that was not entirely true (S4)."""
    ...
    for field_name, raw in patch.items():
        ...
        if k is None:
            rejected[field_name] = "unknown setting"
            continue
        if raw is None:
            applied[field_name] = None  # unset
            continue
        value = _coerce_knob(k.kind, raw)
        if value is None:
            rejected[field_name] = "blank or invalid value — left unchanged"
            continue
        if isinstance(value, (int, float)) and not isinstance(value, bool) and value < 0:
            rejected[field_name] = "must be >= 0"
            continue
        if k.choices is not None and str(value) not in k.choices:
            rejected[field_name] = f"must be one of {', '.join(k.choices)}"
            continue
