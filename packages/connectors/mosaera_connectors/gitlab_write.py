...
from mosaera_connectors._shared import push_repository_to
from mosaera_connectors.gitlab import _push_url
...
def create_project(
    gitlab_url: str, oauth_access_token: str, name: str, *, namespace_id: int | None = None
) -> tuple[dict[str, Any] | None, str | None]:
    """Create a project on the operator's behalf (ADR-0125), authorized by their OAuth token.

    The GitLab counterpart to `github_write.create_public_repo`, and simpler in the way that
    matters: GitLab's `api` scope — the one ADR-0104's connect already asks for — can create a
    project, so this needs no second application. GitHub needed one because its App tokens are
    refused by the equivalent endpoint.

    **Created PRIVATE**, unlike the GitHub path. That is not an inconsistency to fix: `clone.py`
    injects a credential for the configured GitLab host, so a private GitLab project can be cloned
    and its runs start normally. GitHub is public-only because that same injection does not exist
    for it yet. Each provider gets the most private option it can actually deliver, rather than
    the worse of the two for symmetry's sake.

    ``namespace_id`` targets a group; omitted, GitLab creates it under the user's own namespace.
    """
    body: dict[str, Any] = {
        "name": name,
        "path": name,
...
def push_existing_project(
    source_path: Path, *, gitlab_url: str, project_path: str, token: str
) -> tuple[str | None, str | None]:
    """Push a project's history into a GitLab project just created for it (ADR-0125).

    The GitLab spelling of the credentialed URL (``oauth2:``); the push itself is the same
    ``_shared.push_repository_to`` the GitHub path uses.
    """
    return push_repository_to(source_path, _push_url(gitlab_url, project_path, token))
