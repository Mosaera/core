...
def update_pull_request(
    ...
    *,
    ...
    """Patch an already-open pull request's title/body/base.

    Mirrors GitLab's ``update_merge_request``: the idempotent-open path above finds an
    existing PR from the branch and returns early, which used to silently drop any
    title/body/target the operator had just chosen in the compose sheet — the same S3 defect
    GitLab's REST path already handled (it patches ``target_branch`` on the existing-MR
    branch too). Only non-``None`` fields are sent, so a caller that has nothing new to say
    need not resend the old values.
    """
    ...
    if not payload:
        return None, None
    return _api("PATCH", api_base, token, f"repos/{owner_repo}/pulls/{number}", payload)
