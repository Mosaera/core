...
from mosaera_connectors._shared import request_body, request_title
...
def push_existing_repository(
    ...
    This is the half that makes "create a repository for my project" mean something. Creating
    an empty repository and repointing a project at it would leave the project's next run
    cloning nothing — worse than the dead end it replaced. So the push runs FIRST and the
    caller only repoints on success.

    **It reads the source and writes only to the remote.** The push targets an explicit URL
    rather than a named remote, so nothing is added to the source repository's config and no
    remote-tracking ref is created — the project's own checkout is left exactly as found. That
    matters here more than usual: ``source_path`` is a real directory of the operator's, not a
    throwaway workspace.

    Returns ``(branch_pushed, None)`` or ``(None, error)``. Same credential hygiene as
    ``push_branch``: the URL carrying the token is built separately from anything returned,
    every subprocess is bounded, and stderr is scrubbed and capped.
    """
    git = shutil.which("git") or "git"
    try:
        ...
        if branch == "HEAD":  # detached — name the destination explicitly rather than guess
            branch = "main"

        pushed = subprocess.run(  # noqa: S603 — argv list, no shell
            [git, "push", push_url(owner_repo, token, host=host), f"HEAD:refs/heads/{branch}"],
            cwd=source_path,
            capture_output=True,
            text=True,
            timeout=_PUSH_TIMEOUT,
