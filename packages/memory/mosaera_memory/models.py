...
# deny-by-default treatment as ``CHARTER_POSTURES``: the store rejects anything outside the set, so
# a typo can never become a stored run mode. Kept in sync with the API's `RunItemBody.mode` Literal
# by `test_run_modes_in_sync` — memory is a leaf and cannot import apps/api, so a test pins it.
...
class Project(Base):
    ...
    # Internal project understanding, synthesized from the Quincy intake at backlog time.
